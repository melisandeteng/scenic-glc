...
def get_config(runlocal=''):
  ...
  #config.experiment_name = 'glc_vit_finetune'
  # Dataset.
  config.experiment_name = 'imagenet-vit'
      # Dataset.
  config.dataset_name = 'imagenet'
  #config.dataset_configs.dataset = "glc_dataset"#'imagenet2012'#config.dataset_name = 'glc_dataset'
  config.data_dtype_str = 'float32'
  config.dataset_configs = ml_collections.ConfigDict()
  #config.dataset_configs.dataset = "glc_dataset"#'imagenet2012'
  config.dataset_configs.dataset = 'imagenet2012'
  config.no_comet = False
  config.comet= {"tags":["vit", "imagenet", "finetune"]}
  config.dataset_configs = ml_collections.ConfigDict()
  ...
  config.model.representation_size = {'Ti': 192,
                                      ...
                                      'L': 1024,
                                      'H': 1280}[version]
  config.model.classifier = "token" #'token'
  ...
  config.init_from = {'checkpoint_path': "/mnt/disks/persist/checkpoints_vit/", "model_config":"/home/melisande/scenic-glc/scenic/baselines/configs/imagenet/imagenet_vit_config.py"}
