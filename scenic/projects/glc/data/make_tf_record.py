...
def image_example(
    ...
    coordinates,
    label,
    root="/network/scratch/s/sara.ebrahim-elkafrawy/",
):
...
def test_image_example(
    observation_id, coordinates, root="/network/scratch/s/sara.ebrahim-elkafrawy/"
):
    observation_id = str(observation_id)
    ...
    altitude_filename = filename.with_name(filename.stem + "_altitude.tif")
    altitude = tifffile.imread(
        altitude_filename
    ).tobytes()  # open(altitude_filename, 'rb').read()
...
if __name__ == "__main__":
    ...
    init = init_dataset(
        Path("/network/scratch/s/sara.ebrahim-elkafrawy/", region="both"),
        subset="train",
    )

    observation_ids, coordinates, targets = init
    record_file = "/network/scratch/t/tengmeli/temp_glc/val_images_new7.tfrecords"
